...
class GoogleWorkspaceService:
    def __init__(self, token_path: Optional[Path] = None):
        ...
        self._token_path_str = str(token_path) if token_path else str(self.root / "kaedra" / "config" / "google_token.json")
        self._creds = None
        self._services = {}
    ...
    def creds(self):
        ...
        if not self._creds:
            try:
                path = Path(self._token_path_str)
                if path.exists():
                    self._creds = Credentials.from_authorized_user_file(self._token_path_str)
                else:
                    # print(f"[info] Google token not found at {path}")
                    pass
    ...
    def _get_service(self, name: str, version: str):
        ...
        if key not in self._services:
            self._services[key] = build(name, version, credentials=self.creds)
        return self._services[key]

    def __getstate__(self):
        """Exclude unpicklable creds and service objects."""
        state = self.__dict__.copy()
        if "_creds" in state: del state["_creds"]
        if "_services" in state: del state["_services"]
        return state
