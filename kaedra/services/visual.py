...
@dataclass
class VisualResult:
    """Result from a visual generation."""
    file_path: Path
    model: str
...
class VisualService:
    ...
    def generate_video(self,
        ...
        Args:
            ...
            first_frame: Optional first frame image (alias for image, for interpolation clarity)
            last_frame: Optional last frame image (for interpolation)
            input_video: Optional input video object or path (for extension)
            resolution: Output resolution ("720p", "1080p")
            aspect_ratio: Aspect ratio ("16:9", "9:16")
            negative_prompt: Elements to exclude from video
        ...
        try:
            ...
            elif last_frame and start_image:
                # Interpolation mode
                operation = self.client.models.generate_videos(
                    model=self.model,
                    prompt=prompt,
